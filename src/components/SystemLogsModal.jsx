...
export default function SystemLogsModal({ isOpen, onClose }) {
  ...
  const displayOrders = globalSearchResults !== null 
    ? globalSearchResults 
    : (orderSearch.trim() ? recentOrders.filter(o => String(o.id || '').includes(orderSearch.trim())) : recentOrders);
        ...
        {activeTab === 'orders' && (
          <div className="p-3 border-b border-slate-100 dark:border-slate-800">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="ابحث بالرقم، الموبايل، أو البوليصة..."
                value={orderSearch}
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-3">
          {loading ? (
            ...
            displayOrders.length === 0 ? (
              <div className="text-center text-slate-500 text-sm py-10 font-bold bg-slate-50 rounded-xl border border-dashed border-slate-200">
                {orderSearch ? 'لا توجد نتائج لهذا البحث' : 'لا توجد أوردرات'}
              </div>
            ) : (
                        ...
                        orderHistories[order.id]?.map((h, idx) => (
                          <div key={idx} className="border-b border-slate-200 pb-1.5 mb-1.5 last:border-0 last:pb-0 last:mb-0">
                            <div className="flex justify-between items-center mb-1">
                              ...
                              <span dir="ltr" className="text-[9px] font-bold text-slate-400">{new Date(h.created_at).toLocaleString('ar-EG', {dateStyle:'short', timeStyle:'short'})}</span>
                            </div>
                            <div className="text-[10px]">
                              {h.changed_fields?.status && (
                                <p className="text-slate-600">
                                  الحالة: <span className="line-through text-rose-500">{h.changed_fields.status.old}</span> <span className="text-slate-400 mx-1">←</span> <span className="text-emerald-600 font-bold">{h.changed_fields.status.new}</span>
                                </p>
                              )}
                              {h.changed_fields?.trackingNumber && (
                                <p className="text-slate-600 mt-0.5">
                                  البوليصة: <span className="line-through text-rose-500">{h.changed_fields.trackingNumber.old}</span> <span className="text-slate-400 mx-1">←</span> <span className="text-emerald-600 font-bold">{h.changed_fields.trackingNumber.new}</span>
                                </p>
                              )}
                            </div>
                          </div>
