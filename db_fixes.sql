...
-- بياخد أول مجموعة أرقام في الـ id: لو شيلنا كل الحروف ولزقنا الأرقام مع بعض،
-- أي صف رقمه فيه أكتر من رقم (زي "4705/4706/4707") بيطلع عدد خانات أكبر من
-- bigint والاستعلام بيقع بخطأ 22003 out of range.
--
--   SELECT page,
--          COUNT(*) AS عدد_الأوردرات,
--          MAX(
--            CASE WHEN substring(id from '\d+') ~ '^\d{1,18}$'
--                 THEN substring(id from '\d+')::bigint
--            END
--          ) AS أعلى_رقم
--   FROM orders
--   GROUP BY page
...
-- ── 3.4 أوردرات رقمها غريب ───────────────────────────────────────────────────
-- خانة رقم الأوردر في شاشة الإضافة نص حر، فوارد إن حد يكتب فيها كذا رقم مع بعض
-- (زي "4705/4706/4707") — يعني كذا أوردر متجمّعين في صف واحد. الاستعلام ده
-- بيطلّعهم. (ظهر أول مرة كخطأ 22003 وإحنا بنشغّل استعلام 3.3.)
--
--   SELECT id, page, customer, status, date, created_at
--   FROM orders
--   WHERE length(regexp_replace(id, '\D', '', 'g')) > 10
--   ORDER BY created_at DESC;
